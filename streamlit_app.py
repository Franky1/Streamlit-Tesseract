...
try:
    ...
    st.error(
        "TesseractNotFoundError: Tesseract is not installed. Please install Tesseract."
    )
    st.stop()
...
try:
    ...
    st.error(
        "TesseractError: Tesseract reported an error during language data extraction."
    )
    st.stop()
except pytesseract.TesseractNotFoundError:
    st.error(
        ...
        if st.button("Extract Text"):
            ...
            with st.spinner("Extracting Text..."):
                try:
                    ...
                    st.error(
                        "TesseractError: Tesseract reported an error during text extraction."
                    )
                    st.error(f"Error Message: {e}")
                    st.stop()
                except pytesseract.TesseractNotFoundError:
                    ...
                    st.error(
                        "RuntimeError: Tesseract timed out during text extraction."
                    )
                    st.stop()
                except Exception as e:
                    st.error("Unexpected Exception")
